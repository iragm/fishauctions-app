...
- **ci.yml** — PRs + master/main pushes: pub get, generated-code freshness check, `dart format` check, `flutter analyze`, `flutter test`. The steps themselves live in the composite action `.github/actions/flutter-verify` so `dependencies.yml` can run *the same* checks — it has to run them itself, since a branch pushed with `GITHUB_TOKEN` never triggers `ci.yml`.
