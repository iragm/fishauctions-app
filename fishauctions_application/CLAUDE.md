...
- **Multi-lot works end to end** (Part W): `fishauctions://print/?lots=…` is one connect and one `labels/batch/` loop — post what's left, print what comes back, and the next chunk is fetched while the printer works (`_LabelSource`). The *window* sent, not the server's `remaining`, decides what's asked for again. A single reprint stays one `labels/<pk>/` GET; a 404 or unreadable batch answer falls back to per-label GETs for the process. The backend gates in the **view**, so every bulk entry point routes to the printer.
- **What came out is judged from the printer's status, never from bytes written** (`PrintRunLedger`). No profile has a per-label ack. A label counts as printed once a later status read finds the printer idle and clear; a blocked read (jam, cover, paper) fails everything sent since the last clean read, and those go to `labels/printed/` as `failed` + `conditions`, so "print unprinted labels" reprints exactly them. The last label gets one more read once the printer stops printing (≤4 s). A printer that can't be asked reports what it was sent as printed, which is all it can honestly say. Retry resends `LabelPrintResult.retryLots`, not "everything past the printed count".
- **The VEVOR Y486BT can say what state it's in, never that a label came out.** It answers `<ESC>!?` immediately (verified 2026-07-26; lid-open measured as `0x07`), but TSPL has no completion ack. So a jam can only be caught *after* the fact, by the next status read, and **whether it reports a physical jam as `02` is still unverified on hardware**.
- **A printer that never answers a status query is asked once per connection** (`_silentLink`, keyed on `BluetoothService.linkId`); otherwise every label waited out the 5 s timeout.
- **Android asks for the high-priority connection interval** (`_requestFastLink`). Acknowledged writes wait a connection event each, so the interval sets throughput: ~170 writes for a 3×2″ label at the default 30–50 ms is 5 s+ of waiting. iOS has no such control.
- **Every run logs where its time went** (`Label run: … waiting on the server / sending / printer status`). Read that before optimizing anything. If `sending` dominates after all this, the remaining lever is `prefer_write_with_response: false` on the row — a Django edit, but test it with the printer first: unacknowledged writes can overrun a serial module's buffer and truncate labels silently.
...
- **Nothing may throw on the Android GL thread; an exception there kills the app from any screen.** `onDrawFrame` catches everything. The crash on switching apps and resuming: the camera texture and display geometry were handed to the session once, at surface creation, but a session created *later* never got a texture, and ARCore's `update()` throws for that. A late session is the normal case, because ARCore's first availability check answers "checking" and the installer returns via a resume. `ArSessionManager` now records both and applies them to whichever session `update()` finds (fixed 2026-09-11, **unverified on hardware**; if it still crashes, `adb logcat -b crash` names the thread).
- Same pass: availability is retried every 200 ms rather than only on a resume (it was a permanent spinner until one); a session that fails to resume is closed rather than leaked; ML Kit gets a luma copy and the ARCore image is closed at once (it used to outlive a pause or close on ML Kit's thread, a native use-after-free); `Activity.display` is API 30+, so `currentDisplayRotation` falls back below that; the view factory only forgets the *current* view on dispose.
...
- **Values are written from a settled partial; actions wait for the final** (`commit_after_ms`, default 700, served; `0` means finals only, which is the kill switch). The final waits out the 3 s silence window, and that window stays long because every utterance end costs a restart's deafness that swallows the next anchor. So "lot one" took 5–6 s to fill a field. A `sold` seen in a settled partial calls `finishUtterance()` to get the final *now*. A final that disagrees writes the correction. A final whose commands had all already been sent goes to the page as `partial: true`, so the page's local-match fallback doesn't fill the fields twice or save twice.
- **Only one of four phrase endings produces a final result, and set-winners' actions act on finals only** — so a whole command could be heard and discarded. `_flushPendingAsFinal` promotes the last partial; this is why "lot five" never filled anything while longer phrases worked.
